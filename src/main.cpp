...
#if defined(NDEBUG)
# error "Elitecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2764333f934ec7145130036d4d19a6633b0ead148c6fabeb1122e18cf5e3855b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Elitecoin: starting difficulty is 1 / 2^12
...
{
    // Elitecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50000000); // Elitecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = .25 * 24 * 60 * 60; // Elitecoin: 3.5 days
static const int64 nTargetSpacing = .25 * 60; // Elitecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Elitecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Elitecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x25856d113d3105fb36e8a7e2003a5cf0a1170497cbe39a7c1b0e6d63d2e76846");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cristmas markets slump";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 400 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047167aca8dd40e4570cd98a21efd3f4ead6c5c26c4153ec00b845dde549240c86dc69af6125c41ccf6d338f931aa16f4bbae72bb871cd9cad45761cf6ab52c751") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1546027508;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084729358;

        if (fTestNet)
        {
            block.nTime    = 1546027455;
            block.nNonce   = 386400170;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xeb7355f6db0ecb42d9d81b4ec0b5785b6957a1f7d6fe32fc5fa2ff9cd5ac44ca"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc3, 0xba, 0xd2 }; // Elitecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ElitecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ElitecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ElitecoinMiner(CWallet *pwallet)
{
    printf("ElitecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ElitecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ElitecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ElitecoinMiner, pwallet));
}
